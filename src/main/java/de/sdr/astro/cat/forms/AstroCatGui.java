...
public class AstroCatGui {
    ...
    private JDialog exporterDialog;
...
//        PathObjectTreeNode aoNode = new PathObjectTreeNode(astroObject.getName(), Model.ASTRO_OBJECT, new AstroObject(textFieldPath.getText()));
    ...
    public void showImageExporterDialog() {
        if (exporterDialog == null) {
            exporterDialog = new JDialog(this.getMainFrame());
            exporterDialog.setTitle(Config.getInstance().getL10n().getString("exporter_title"));
            imageExportPanel = new ImageExportPanel();
            exporterDialog.setContentPane(imageExportPanel.getTopPanel());
            exporterDialog.pack();
